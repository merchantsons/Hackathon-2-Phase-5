...
import { apiService, taskToTodo } from '../services/api';
...
export const useTodos = () => {
  ...
  const updateTodo = async (id: string, updates: Partial<Todo>) => {
    ...
    if (updates.recurrenceRule !== undefined) updatePayload.recurrence_rule = updates.recurrenceRule ?? null;
    if (updates.reminderAt !== undefined) updatePayload.reminder_at = updates.reminderAt ?? null;
    ...
    try {
      ...
      const todos: Todo[] = apiTasks.map((task: any) => taskToTodo(task) as Todo);
