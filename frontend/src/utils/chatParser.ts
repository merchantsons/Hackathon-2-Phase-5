...
export function parseCommand(message: string): ParsedCommand {
  ...
  // Phase V: extract tags (e.g. "tagged work, urgent", "tags: a b", or #work #urgent)
  let tags: string[] | undefined;
  const taggedMatch = message.match(/(?:tagged|tags?)\s*[:\s]+([^\n,.]+)/i);
  if (taggedMatch?.[1]) {
    tags = taggedMatch[1].split(/[\s,]+/).map((t) => t.trim()).filter(Boolean);
  } else {
    const hashTags = message.match(/#(\w+)/g);
    if (hashTags?.length) tags = hashTags.map((t) => t.replace(/^#/, ''));
  }

  // Phase V: recurrence (e.g. "recurring daily/weekly/monthly")
  let recurrenceRule: 'daily' | 'weekly' | 'monthly' | undefined;
  if (/\brecurring\s+daily\b/i.test(message) || /\bdaily\s+recurring\b/i.test(message)) recurrenceRule = 'daily';
  else if (/\brecurring\s+weekly\b/i.test(message) || /\bweekly\s+recurring\b/i.test(message)) recurrenceRule = 'weekly';
  else if (/\brecurring\s+monthly\b/i.test(message) || /\bmonthly\s+recurring\b/i.test(message)) recurrenceRule = 'monthly';

  // Phase V: reminder (e.g. "remind 60 minutes before", "remind me 30 min before")
  let reminderMinutesBefore: number | undefined;
  const remindMatch = message.match(/\bremind\s+(?:me\s+)?(\d+)\s*(?:min(?:ute)?s?)?\s*before\b/i);
  if (remindMatch && remindMatch[1]) reminderMinutesBefore = parseInt(remindMatch[1], 10);
      ...
      case 'create': {
        if (!command.taskTitle || command.taskTitle === 'New Task') {
          ...
          try {
            ...
            // Todo Assistant defaults: reminder = now + 1 min, due = tomorrow, recurrence = daily
            const now = new Date();
            const reminderAt = new Date(now.getTime() + 60 * 1000).toISOString();
            ...
            await apiService.createTask({
              ...
              due_date: command.dueDate || dueDate,
              tags: command.tags?.length ? command.tags : undefined,
              recurrence_rule: command.recurrenceRule || 'daily',
            ...
            response += `\nDue date: ${command.dueDate || dueDate}`;
