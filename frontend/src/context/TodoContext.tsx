...
import { apiService, Task, taskToTodo } from '../services/api';
  ...
  switch (action.type) {
    ...
    case 'ADD_TODO': {
      ...
      apiService.createTask({
        ...
        tags: newTodo.tags?.length ? newTodo.tags : undefined,
        recurrence_rule: newTodo.recurrenceRule || undefined,
        reminder_at: newTodo.reminderAt ?? undefined,
      })
        .then(async () => {
  ...
  useEffect(() => {
    const loadTodosFromAPI = async () => {
      try {
        ...
        // Backend returns an array; guard against non-array (e.g. wrapper object)
        const apiTasks = Array.isArray(raw) ? raw : (raw && (raw as any).tasks) || [];
        const todos: Todo[] = apiTasks.map((task: Task) => taskToTodo(task) as Todo);
