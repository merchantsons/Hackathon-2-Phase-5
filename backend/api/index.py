...
app = FastAPI(title="Todo API - Phase 5", version="1.0.0")
...
try:
    ...
    if 'tasks' in existing_tables:
        with Session(engine) as session:
            ...
            # Required columns from Task model (including Phase V)
            required_columns = {
            ...
            # Phase V: ensure optional columns allow NULL (fix NOT NULL constraint if present)
            nullable_columns = ['recurrence_rule', 'parent_task_id', 'reminder_minutes_before', 'reminder_at', 'due_date', 'description']
            for col_name in nullable_columns:
                try:
                    session.exec(text(f"ALTER TABLE tasks ALTER COLUMN {col_name} DROP NOT NULL"))
                    session.commit()
                    print(f"✅ Dropped NOT NULL on tasks.{col_name}", file=sys.stderr, flush=True)
                except Exception as e:
                    session.rollback()
